...
#ifndef CPPSTUDY_PIGGY_H
#define CPPSTUDY_PIGGY_H
#include <cmath>
#include <cstdio>
#include <cstdlib>
#define T double
...
T f(T x,T y){ return o(x,y,no,1)
                     ? fmax(o(x,y,no,1), s(x,y,nh,12))
                     : fmax(o(x,y,fa,1), fmax(o(x,y,ey,11), fmax(o(x,y,ea,1), fmax(o(x,y,mo,1), fmax(s(x,y,fr,13), s(x,y,pu,12))))));}

void drawPiggy(int a, char**b) {
    for(T y= -1, s = a > 1? strtod(b[1], nullptr) : 1; y < 0.6; y += 0.05/s, putchar('\n'))
        for(T x=-1; x < 0.6; x += 0.025/s)
            putchar(" .|/=\\|/=\\| @!" [(int)f(u(x,y,0.3),v(x,y,0.3))]);
}
#endif //CPPSTUDY_PIGGY_H
